...
from .hypercore.nn import LorentzLinear, LorentzActivation
...
def project(manifold, x):
    x_space = x
    x_time = ((x_space ** 2).sum(dim=-1, keepdims=True) + manifold.c).clamp_min(1e-4) ** 0.5
...
class LorentzNonLinearHead(nn.Module):
    def __init__(self, manifold, input_dim, out_dim, activation_fn, hidden=None, manifold_out=None, entailed=None):
        super(LorentzNonLinearHead, self).__init__()
        self.manifold = manifold
        self.entailed=entailed
        hidden = input_dim if not hidden else hidden
        if manifold_out is None:
            self.manifold_out = [self.manifold]
        else:
