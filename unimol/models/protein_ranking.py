...
class ProteinRankingModel(BaseUnicoreModel):
    ...
    def __init__(self, args, mol_dictionary, pocket_dictionary, manifold_in, manifold_out=None, entailed=[False]):
        ...
        self.protein_project = LorentzNonLinearHead(
            self.protein_model.config.hidden_size, 128, "relu", manifold_out=manifold_out, entailed=reversed_entailed
        )
        ...
        # self.w = nn.Parameter(torch.tensor([0.75, 1.0, 1.25, 1.5]))
    ...
    def forward(
        ...
        # project to input manifold
        mol_rep = project(self.manifold, mol_rep)
        protein_rep = project(self.manifold, protein_rep)

        # project to latent manifolds
