...
class Lorentz(LorentzOri):
    ...
    def oxy_angle(self, x, y, eps: float = 1e-4, c=None):
        """
    ...
    def half_aperture(self, x, min_radius: float = 0.1, eps: float = 1e-4, c=None):
        """
        ...
        if c is not None:
            asin_input = 2 * min_radius / (torch.norm(x[..., 1:], dim=-1) * (1/c)**0.5 + eps)
        else:
